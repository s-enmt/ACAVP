...
class MinMaxVals:
    ...
    translate: MinMax = field(default_factory=lambda: MinMax(0, 10))
    rotate: MinMax = field(default_factory=lambda: MinMax(0, 30))
    solarize: MinMax = field(default_factory=lambda: MinMax(0, 256))
    posterize: MinMax = field(default_factory=lambda: MinMax(0, 4))
...
class TrivialAugmentDataset(Dataset):
	"""TrivialAugment augmentation"""
	def __init__(self, dataset, post_transform):       
		self.dataset = dataset
		self.post_transform = post_transform

		# Initialize TrivialAugment
		self.augmenter = TrivialAugment()
...
class RandAugmentDataset(TrivialAugmentDataset):
	"""RandAugment augmentation"""
	def __init__(self, dataset, post_transform):  
		super().__init__(dataset, post_transform)
